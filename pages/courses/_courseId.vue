    ...
    <div class="flex justify-between max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <span class="inline-flex rounded-md shadow-sm">
        <button
          ...
          type="button"
          class="inline-flex items-center px-4 py-1 border border-red-500 text-sm leading-6 rounded-md text-red-500 hover:bg-red-500 hover:text-white focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-indigo-700 transition ease-in-out duration-150"
        >
      ...
      <!-- <div class="bg-white rounded-lg shadow py-6 px-10">
        <div class>
            ...
            <form @submit.prevent>
              <div class="grid grid-cols-6 gap-6">
                <div class="col-span-6">
                  <label for="name" class="block text-sm font-medium leading-5 text-gray-700">Course Name</label>
                  <input
                    v-model="course.name"
                    id="name"
                ...
                <div class="col-span-6">
                  <div class="flex justify-between">
                    <label for="teacherName" class="block text-sm font-medium leading-5 text-gray-700">Teacher</label>
                  </div>
                  <div>
                    <input
          ...
          <div class="flex-1">
            ...
            <div v-for="(breakdown, index) in course.weightBreakdown" :key="index" class="grid grid-cols-6 gap-x-4">
            ...
            <span class="mt-4 col-span-6 flex w-full rounded-md shadow-sm">
              <button
                @click="course.weightBreakdown.push({name: null, weight: null})"
                type="button"
      ...
      <div class="flex flex-col space-y-6">
        ...
        <div class="bg-white rounded-lg shadow py-6 px-10">
          <div class="flex justify-between max-w-7xl mx-auto<div">
            ...
            <!-- <span class="inline-flex rounded-md shadow-sm">
              <button
                ...
                type="button"
                class="inline-flex items-center px-4 py-1 border border-transparent text-sm leading-6 rounded-md text-white bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-indigo-700 transition ease-in-out duration-150"
              >
          ...
          <form @submit.prevent>
            ...
            <div>
              <h3 class="mt-6 text-2xl font-semibold text-gray-900 col-span-6 mb-4">Grade Breakdown</h3>
              <div v-for="(breakdown, index) in course.weightBreakdown" :key="index" class="grid grid-cols-6 gap-x-4">
                <div :class="[index > 0 ? 'mt-3' : 'mt-1', 'col-span-4']">
                  <input
                    class="form-input block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5"
              ...
              <table class="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr>
                    ...
                    <th class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Date Added</th>
                    <th class="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Grade</th>
                    <th class="px-6 py-3 bg-gray-50"></th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(grade,index) in grades" :key="grade.name" :class="index % 2 === 0 ? 'bg-white' : 'bg-gray-50'">
                    <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 font-medium text-gray-900">{{grade.name}}</td>
                    <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-500">{{grade.type}}</td>
                    <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-500">some date</td>
                    <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-500">{{grade.grade}}</td>
                    <td class="px-6 py-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                      <button @click="gradeModal = 'edit'; addEditGrade = grade" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        >
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-headline">Delete Course</h3>
              <div class="mt-2">
                <p
                  class="text-sm leading-5 text-gray-500"
                >Are you sure you want to delete this course? All of your data will be permanently removed from our servers forever. This action cannot be undone.</p>
              </div>
            </div>
          </div>
...
To: "opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
        ...
        >
          ...
          <div class="flex justify-between border-b items-center pb-3">
            <h2 id="modal-title" class="text-xl font-semibold text-gray-900">{{gradeModal === 'add' ? 'Add' : 'Edit'}} Grade</h2>
            <span v-if="gradeModal === 'edit'" class="inline-flex rounded-md shadow-sm">
              <button
                @click="deleteGrade"
                type="button"
                class="inline-flex items-center px-4 py-1 border border-red-500 text-sm leading-6 rounded-md text-red-500 bg-white hover:bg-red-500 hover:text-white focus:outline-none focus:border-indigo-700 focus:shadow-outline-indigo active:bg-indigo-700 transition ease-in-out duration-150"
          ...
          <form class="mt-4" @submit.prevent>
            <div class="grid grid-cols-6 gap-4">
              ...
              <div class="col-span-6">
                ...
                <div>
                  ...
                  >
                    <option v-for="breakdown in course.weightBreakdown" :key="breakdown.name">{{breakdown.name}}</option>
                  </select>
                </div>
              </div>
              <div class="col-span-6">
            ...
            <div class="flex mt-5 sm:mt-6 space-x-4 border-t pt-4">
              <span class="flex w-full rounded-md shadow-sm">
                <button
                  @click="gradeModal = false; newCourse =  {name: null,teacher: {name: null,email: null,phone: null}, weightBreakdown: [{name: null,weight: null,}]}"
                  type="button"
